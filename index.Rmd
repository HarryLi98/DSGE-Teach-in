...
### Motivation
- DSGE models are widely used in macroeconomic research and appear in almost every substantive area of macroeconomics, such as the theory of investment, the theory of the consumer, search theory, public finance, growth theory and most importantly, they appear in the overlapping generations model for life-cycle consumption and housing demand that Matt and I are working on. One of the reasons that DSGE models are attractive is that these models are derived from micro foundations - that is households with rational expectations maximizing utility, firms maximize profits, in general equilibrium. 
...
???
### Overview
There are many types of DSGE model structures and I'll talk briefly about some of these structures and their relative advantages and disadvantages. The methods required for solving these models also differ by model structure.
...
Control variables are usually easy to identify. 
--
In this example, the control variables are $c_t, s_t$.
--
...
State variables are more difficult to identify. 
--
In this example, there is an endogenous state variable $s_{t-1}$ and an exogenous state variable $y_{t}$.
...
Take first order conditions with respect to control variables $c_{t}, s_{t}$ and co-state $\lambda_{t}$:
...
Take first order conditions with respect to control variables $c_{t}, n_{t}, a_{t}$ and co-state $\lambda_{t}$:
